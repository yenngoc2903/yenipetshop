...
<body>
  ...
  <?php 
    include('../connect/connect.php');
    $sql_sua_loaithucung = "select * from loaithucung where maLoai='$_GET[maloai]' limit 1";
    $query_sua_loaithucung = mysqli_query($mysqli, $sql_sua_loaithucung);
  ?>
   <p>Sửa loại thú cưng</p>
    <table class="quanly_table" >
      <form method="POST" action="xuly.php?maloai=<?php echo $_GET['maloai'] ?>">
        <?php
          while($dong = mysqli_fetch_array($query_sua_loaithucung)) {
        ?>
        <tr class="item">
          <td>Tên loại thú cưng</td>
          <td> <input type="text" name="tenLoai" value="<?php echo $dong['tenLoai'] ?>"> </td>
        </tr>
        <tr class="item">
          <td colspan="2"><input class="submit" type="submit" name="sualoaithucung" value="Sửa loại thú cưng" ></td>
        </tr>
      <?php
          }
      ?>
